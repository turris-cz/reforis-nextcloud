...
lint-web: venv
	$(VENV_BIN)/$(DEV_PYTHON) -m pylint --rcfile=pylintrc reforis_nextcloud
	$(VENV_BIN)/$(DEV_PYTHON) -m pycodestyle --config=pycodestyle reforis_nextcloud
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_nextcloud/translations/messages.pot .
init-langs: create-messages
	...
	; done
update-messages:
	$(VENV_BIN)/pybabel update -i ./reforis_nextcloud/translations/messages.pot -d ./reforis/translations
compile-messages:
	$(VENV_BIN)/pybabel compile -f -d ./reforis_nextcloud/translations

clean:
	find . -name '*.pyc' -exec rm -f {} +
	rm -rf $(VENV_NAME) *.eggs *.egg-info dist build .cache
	rm -rf dist build *.egg-info
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_nextcloud/js/app.min.js
	$(ROUTER_PYTHON) -m pip uninstall -y reforis_nextcloud
